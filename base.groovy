pipeline {
  ...
  parameters {
    string(
      ...
      defaultValue:'758526784474.dkr.ecr.us-east-1.amazonaws.com/base-nodejs-bkd-qa:develop',
          ...
          catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
            ...
            configFileProvider([configFile(fileId: "${params.env}", variable:"${params.env}")]) {
              wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'xterm']) {
                sh "cd /base-nodejs-bkd-qa && . \$${params.env} >> /dev/null && ${params.command_run}"
              }
            }
          }
        }
    ...
    post {
      always{
      echo "TESTS FINISHED"
        sh 'cd /base-nodejs-bkd-qa && zip -r allure-results allure-results'
        sh 'cp /base-nodejs-bkd-qa/allure-results.zip ${WORKSPACE}/'
        sh 'cd /base-nodejs-bkd-qa && rm -R allure-results.zip'
        sh 'cd ${WORKSPACE} && unzip allure-results.zip'
        sh 'cd ${WORKSPACE} && rm -R allure-results.zip'
        sh 'mkdir -p ${WORKSPACE}/reports/playwright-report && cp -R /base-nodejs-bkd-qa/reports/playwright-report/* ${WORKSPACE}/reports/playwright-report'
